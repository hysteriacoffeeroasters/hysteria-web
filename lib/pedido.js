...
/* ---------------------------------------------------------------------------
   ...
   El cuarto campo es opcional. 'unico' = una sola vez por persona, medido por
   el correo con el que se paga. Ejemplos inventados, NO los códigos vivos —
   este archivo se sirve público y escribir aquí uno real lo regalaría:
...
function codigosSecretos() {
  ...
  for (const linea of bruto.split(/[\n,]+/)) {
    ...
    salida[codigo] = {
      ...
      valor: Number(valor),
      activo: true,
      unicoPorPersona: String(marca || '').toLowerCase() === 'unico',
    };
  }
  return salida;
}
...
export function leerCodigo(bruto) {
  ...
  if (tipo === 'enviogratis') return { codigo, tipo, valor: 0, unicoPorPersona };
  ...
  return { codigo, tipo, valor, unicoPorPersona };
}
...
export function construirPedido(entrada, codigoBruto) {
  ...
  /* El descuento nunca supera el subtotal (un total negativo no existe) y
     el umbral del envío gratis se mide con lo que de verdad se paga. */
  const cupon = leerCodigo(codigoBruto);
  let descuento = 0;
  if (cupon && subtotal > 0) {
    if (cupon.tipo === 'porcentaje') descuento = Math.round(subtotal * cupon.valor / 100);
    if (cupon.tipo === 'fijo')       descuento = Math.min(cupon.valor, subtotal);
  }
  const subtotalReal = subtotal - descuento;

  const costoEnvio =
    (cupon && cupon.tipo === 'enviogratis') ? 0 :
