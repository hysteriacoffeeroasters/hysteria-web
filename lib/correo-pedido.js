...
export async function yaAvisado(referencia) {
  ...
  try {
    const r = await fetch(
      'https://api.brevo.com/v3/smtp/emails?tags=' + encodeURIComponent(referencia) + '&limit=1',
      { headers: { 'api-key': clave.trim(), 'Accept': 'application/json' } });
    if (!r.ok) return false;
    const datos = await r.json();
    return Array.isArray(datos.transactionalEmails) && datos.transactionalEmails.length > 0;
  } catch (err) {
