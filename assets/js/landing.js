...
(function () {
  ...
  <header class="nav" id="nav">
    <a href="${ruta('/')}" class="nav-logo" aria-label="Hysteria Coffee Roasters, ${traducir('inicio')}">
    ...
    <nav class="nav-links landing-nav" aria-label="${traducir('Navegación')}">
      <a href="${ruta('/')}">${traducir('Ver todo el sitio')}</a>
    </nav>
    <div class="nav-actions">
      <button class="cart-btn" id="cart-btn" aria-label="${traducir('Carrito vacío')}">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
          <path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/>
          <path d="M3 6h18M16 10a4 4 0 0 1-8 0"/>
        </svg>
        <span class="cart-label">${traducir('Carrito')}</span>
  ...
  <div class="landing-otras">
    <p class="landing-otras-tit">${traducir('Sigue explorando')}</p>
    <nav class="landing-otras-links" aria-label="${traducir('Otras secciones')}">
      <a href="${ruta('/cafes')}">${traducir('Nuestros cafés')}</a>
      <a href="${ruta('/menu')}">${traducir('Menú')}</a>
      <a href="${ruta('/preparacion')}">${traducir('Cómo prepararlo')}</a>
      <a href="${ruta('/tienda')}">${traducir('Tienda')}</a>
      <a href="${ruta('/visitanos')}">${traducir('Visítanos')}</a>
    </nav>
  </div>
    ...
    <div class="footer-inner">
      <div class="footer-mark">
        ...
        <p class="footer-tagline">
          ${traducir('Café tostado con pasión.')}<br>
          ${traducir('Trato directo y precio justo con el caficultor.')}<br>
          Bogotá, Colombia.
        </p>
      </div>
      <div>
        <p class="footer-col-title">${traducir('Contacto')}</p>
  ...
  <aside class="cart" id="cart" role="dialog" aria-modal="true" aria-hidden="true"
    ...
    <div class="cart-head">
      <h2 id="cart-title">${traducir('Tu pedido')}</h2>
      <button class="cart-close" id="cart-close" aria-label="${traducir('Cerrar carrito')}">
    ...
    <div class="cart-foot" id="cart-foot" hidden>
      ...
      <button class="btn btn-solid cart-checkout" id="cart-checkout">${traducir('Finalizar compra')}</button>
      <p class="cart-nota" id="nota-resumen">${traducir('Cerramos tu pedido por WhatsApp')}</p>
    </div>

    <form class="cart-envio" id="cart-envio" hidden novalidate>
      <div class="envio-head">
        <button type="button" class="envio-volver" id="envio-volver">← ${traducir('Volver al pedido')}</button>
        <p class="envio-titulo">${traducir('¿A dónde lo enviamos?')}</p>
      </div>

      <label class="campo">
        <span>${traducir('Nombre y apellido')}</span>
        <input type="text" id="env-nombre" name="nombre" autocomplete="name" required>
      </label>

      <label class="campo">
        <span>${traducir('Celular / WhatsApp')}</span>
      ...
      <label class="campo">
        <span>${traducir('Indicaciones')} <em>${traducir('(opcional)')}</em></span>
        <input type="text" id="env-notas" name="notas" placeholder="${traducir('Portería, horario, punto de referencia')}">
      </label>

      <p class="envio-error" id="envio-error" role="alert"></p>
      <button type="submit" class="btn btn-solid cart-checkout" id="envio-pagar">${traducir('Ir a pagar')}</button>
      <p class="cart-nota" id="nota-envio">${traducir('Usamos tus datos solo para el envío y tu factura.')}<br>
        ${traducir('Coordinamos el pago contigo por WhatsApp.')}</p>
    </form>
  </aside>

  <a class="wa-float" id="wa-float" href="#" target="_blank" rel="noopener" aria-label="${traducir('Escríbenos por WhatsApp')}">
