...
export function registrarPedidoSinAvisar({ referencia, pedido, dest, transaccion, via }) {
  try {
    console.error('PEDIDO_SIN_AVISAR ' + JSON.stringify({
      ...
      descuento: (pedido && pedido.descuento > 0)
        ? `${pedido.codigo || 'sin código'} (-${pedido.descuento})` : undefined,
      cliente: dest && {
...
/* ── ¿Ya salió el aviso de esta referencia? ───────────────────────────────
   ...
   en esa ventana, salen dos correos con la MISMA referencia. Desde que el
   webhook lee el pedido guardado (lib/guardado.js), los dos pueden llegar con
   el detalle completo y casi idénticos: solo los separa el pie, "Wompi" en el
   del regreso y "Wompi (CARD)" o similar en el del evento. Misma referencia =
   un solo pedido; se despacha una vez. Ante la duda se prefiere un duplicado
   a un pedido sin avisar. */
...
export async function enviarCorreoCliente({ referencia, pedido, dest, sinDetalle }) {
        ...
        <tr>
          <td style="padding:0 40px;">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-top:2px solid #000;">
              ...
              <tfoot>
                ${pedido.descuento > 0 ? `
                <tr><td style="padding:12px 0 0;font-family:Helvetica,Arial,sans-serif;font-size:14px;color:#757575;">Descuento${pedido.codigo ? ' (' + esc(pedido.codigo) + ')' : ''}</td>
                    <td style="padding:12px 0 0;text-align:right;font-family:Helvetica,Arial,sans-serif;font-size:14px;color:#000;">−${pesos(pedido.descuento)}</td></tr>` : ''}
                ${sinDetalle ? '' : `
  ...
  const texto = [
    ...
    pedido.descuento > 0 ? `Descuento${pedido.codigo ? ' (' + pedido.codigo + ')' : ''}: −${pesos(pedido.descuento)}` : '',
    // Sin detalle no se conoce el desglose: afirmar "envío gratis" era mentir
    // a quien sí lo pagó dentro del total.
    sinDetalle ? '' : `Envío: ${pedido.costoEnvio === 0 ? 'gratis' : pesos(pedido.costoEnvio)}`,
    ...
    <div style="padding:24px;">
      ...
      <table style="width:100%;border-collapse:collapse;font-size:14px;">
        ...
        <tfoot>
          ...
          ${pedido.descuento > 0 ? `
          <tr><td colspan="2" style="padding:0 12px 10px;text-align:right;color:#666;">Descuento${pedido.codigo ? ' (' + esc(pedido.codigo) + ')' : ''}</td>
              <td style="padding:0 12px 10px;text-align:right;">−${pesos(pedido.descuento)}</td></tr>` : ''}
