...
export async function reservarCodigoGlobal(codigo, referencia, correo) {
  ...
  try {
    ...
    const previa = await leerReservaGlobal(r);
    if (previa && previa.confirmado === false && huella && previa.huella === huella) {
      try {
        await put(r, contenido(), {
    ...
    /* Si NO se pudo leer la reserva, el fallo no es del cliente sino del
       almacén, y decirle "ese código ya se usó" a alguien legítimo es mentirle.
       Se niega igual —para un código de un solo uso, negar es el lado seguro—
       pero queda distinguido en el registro para no perseguir un fantasma. */
    if (!previa) {
      console.error('RESERVA_NO_LEGIBLE ' + codigo + ': se niega por precaución, ' +
        'puede ser un fallo del almacén y no un código ya usado');
    }
    return false;
  }
}

/* Lee la reserva. null si no existe o no se puede leer — quien llame decide
   qué hacer con esa duda, y en este archivo la duda siempre significa NO tocar. */
async function leerReservaGlobal(ruta) {
  try {
    const r = await get(ruta, { access: 'private', abortSignal: AbortSignal.timeout(3000) });
...
/** Vuelve definitiva la reserva. Solo tras un pago aprobado.
    ...
    EXIGE que exista una reserva de ESTA transacción. Sin esa comprobación,
    cualquiera que llamara a /api/wompi-confirmar con un id aprobado y un
    carrito que cuadrara podía crear el archivo como confirmado y matar el
    código para siempre sin haberlo canjeado — y confirmado no lo deshace ni
    el webhook ni la purga. */
export async function confirmarCodigoGlobal(codigo, referencia) {
  ...
  const previa = await leerReservaGlobal(r);
  if (!previa) {
    console.error('CONFIRM_SIN_RESERVA ' + codigo + ' ref=' + referencia);
    return false;
  }
  // Ya confirmada por esta misma referencia: nada que hacer, y es un éxito
  if (previa.confirmado === true) return previa.referencia === referencia;
  if (previa.referencia !== referencia) {
    console.error('CONFIRM_DE_OTRA_RESERVA ' + codigo + ' ref=' + referencia +
      ' reservadaPor=' + previa.referencia);
    return false;
  }

  try {
    await put(r, JSON.stringify({
      referencia, huella: previa.huella || '',
      confirmado: true, confirmadoEn: new Date().toISOString(),
    }), {
      access: 'private',
      contentType: 'application/json',
      addRandomSuffix: false,
...
/** Suelta la reserva: el pago murió y el código vuelve a estar disponible.
    ...
    Solo suelta LA SUYA y solo si sigue sin confirmar. Borrar a ciegas permitía
    que un pago que muere tarde —un PSE caducado, un efectivo vencido— borrara
    la reserva CONFIRMADA del reintento con tarjeta del mismo cliente, y ahí el
    código quedaba libre después de haberse usado de verdad. */
export async function liberarCodigoGlobal(codigo, referencia) {
  const r = rutaGlobal(codigo);
  if (!r || !hayGuardado()) return false;

  const previa = await leerReservaGlobal(r);
  if (!previa) return false;                       // no existe o no se pudo leer
  if (previa.confirmado !== false) return false;   // confirmada: intocable
  if (referencia && previa.referencia !== referencia) return false;  // no es tuya
